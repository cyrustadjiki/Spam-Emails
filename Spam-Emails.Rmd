...
lasso_results = lasso_cv %>% collect_metrics() %>% group_by(.metric) %>% summarize(mean_accuracy = mean(mean, na.rm = T))

lasso_results[1] = c("Mean Absolute Error", "Root Mean Squared Error")
names(lasso_results) = c("Metric", "Mean Accuracy")
kable(lasso_results, digits = 3) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
